...
if(isset($_GET['keyword']))
	...
	function moving(){
		...
		Top+=2*ratio;
		Left+=2;
		if(Top-12>inputField.offsetTop){
			interval=self.setInterval(function(){typing()},100);
	...
	var tick=0;
	function typing(){
		if((keyWord.charAt(tick))!=""){
			inputField.value=inputField.value+ keyWord.charAt(tick++);
			}
		else
			{
				interval=window.clearInterval(interval);
				intrvl=self.setInterval(function(){clicking()},10);
			}
	}
	//var ratio1=((searchButton.offsetTop-Top)/(searchButton.offsetLeft-Left));
	var ratio1=3.7;
	function clicking(){
		cursor.style.top= Top+"px";
		cursor.style.left= Left+"px";
		Top+=1;
		Left+=ratio1;
		if(Top-20>searchButton.offsetTop){
			cursor.style.left= Left+10+"px";
			intrvl=window.clearInterval(intrvl);
			searchButton.className=searchButton.className+ " hover";
			window.setTimeout(function(){searchButton.className=searchButton.className+ " active";search();},900);
		}
	}
	function search(){
		location.href = "http://www.google.com/search?btnG=1&pws=0&q="+keyWord;
	}
	</script>
	</body>
	</html>';
else
	echo "Error, keyword parameter missing!";
?>